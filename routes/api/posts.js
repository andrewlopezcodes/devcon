  ...
  Profile.findOne({user: req.user.id})
    .then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          if(post.likes.filter(like => like.user.toString() === req.user.id).length === 0){
            return res.status(400).json({alreadyliked: 'User has not already liked this post'})
          }
...
router.post('/comment/:id', passport.authenticate('jwt', {session: false}), (req, res) => {
  ...
  //Check validation
  if(!isValid){
    //if erros accure send 400 with errors object
    return res.status(400).json(errors);
  }
  ...
  Post.findById(req.params.id)
    .then(post => {
      ...
      if(post.comments.filter(comment => comment._id.toString() === req.params.comment._id).length === 0){
        return res.status(404).json({commentdoesntexists: 'Comment does not exist'});
      }
