  ...
  render(){
    ...
    const expItems = experience.map(exp => (
      <li key={exp._id} className="list-group-item">
        ...
        <p>
          <Moment format="ll">{exp.from}</Moment> - {exp.to === null ? ('Now') : (<Moment format="ll">{exp.to}</Moment>)}
        </p>
    ...
    const eduItems = education.map(edu => (
      <li key={edu._id} className="list-group-item">
        ...
        <p>
          <Moment format="ll">{edu.from}</Moment> - {edu.to === null ? ('Now') : (<Moment format="ll">{edu.to}</Moment>)}
        </p>
